...
async fn main() {
    ...
    let mut website: Website = Website::new("https://heygoody.com")
     ...
     // Increase the network idle timeout
     .with_wait_for_idle_network(Some(WaitForIdleNetwork::new(Some(Duration::from_secs(60)))))
     // Increase the overall request timeout
     .with_request_timeout(Some(Duration::from_secs(60)))
     // Add user agent to appear more like a regular browser
     .with_user_agent(Some("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36"))
     .build()
     .unwrap();
